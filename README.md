...
Once data for light curves are gathered and saved in an array, the pipeline will proceed to plotting. The pipeline will print out the standard deviations of the difference of magnitudes of target and comparison star (V-C), and comparison and validation star (C-K) along with the mean error of the target star magnitudes for every aperture radius. Then the pipeline will ask for a number of the aperture radius for which it loads the magnitudes and plots them versus time in Julian date format which is acquired from the FITS header of each image during light curve array generating.
