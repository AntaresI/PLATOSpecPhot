...
def file_and_folder_preparation(configfile):
    ...
    '''ADDING ADU UNIT TO FITS HEADERS OF IMAGES AND CHANGING ZERO ADU PIXELS FOR FLATS'''
    ...
    reduced_darks_path = Path(reduced +'\\reduced\\reduced_darks')
    reduced_darks_path.mkdir(exist_ok=True)
    path_array.append(reduced_darks_path)
    
    reduced_flats_path = Path(reduced +'\\reduced\\reduced_flats')
